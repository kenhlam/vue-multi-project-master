...
export const KY_Get = (url, params, config = {}) => {
    return new Promise(
        (resolve, reject) => {
            ...
            let key = url + JSON.stringify(params) + JSON.stringify(config);
            let lastData = handleData(key);
            if (lastData) {
                return resolve(Object.assign({}, lastData))
            }
...
export const KY_Post = (url, data, config = {}) => {
    ...
    return new Promise((resolve, reject) => {
        ...
        let key = url + JSON.stringify(data) + JSON.stringify(config);
        let lastData = handleData(key);
        if (lastData && !config.refresh) {
            return resolve(Object.assign({}, lastData))
        }
